...
class TestAnalyzer:
        ...
        for line in lines:
            ...
            if "--- PASS:" in line:
                ...
                if match:
                    ...
                    self.phase_results[phase_key]["tests"].append(
                        {"name": test_name, "status": "PASS", "time": test_time}
                    )
            ...
            # Parse FAIL lines: "--- FAIL: TestPhase1DataLoaderSequential (0.05s)"
                ...
                if match:
                    ...
                    self.phase_results[phase_key]["tests"].append(
                        {"name": test_name, "status": "FAIL", "time": test_time}
                    )
                    self.test_times[test_name] = test_time
    ...
    def generate_report(self):
        ...
        report.append(
            f"- **Pass Rate:** {total_pass}/{total_tests} ({100*total_pass/total_tests:.1f}%)"
        )
        report.append(
            f"- **Total Runtime:** {self.total_time:.2f}s ({self.total_time/60:.1f} minutes)"
        )
        report.append("")
        ...
        for phase_num in range(1, 5):
            if phase_counts[phase_num] > 0:
                ...
                report.append(
                    f"- **Phase {phase_num}:** {total_time:.2f}s total, {avg_time*1000:.1f}ms per test"
                )
        ...
        for phase_num in range(1, 5):
            ...
            report.append(
                f"| {phase_num} | {expected_pass}/{expected['total']} | {pass_count}/{total} | {pass_rate:.1%} | {status} |"
            )
        ...
        if total_pass >= total_expected * 0.90:
            ...
            report.append(
                f"Pass rate {total_pass}/{total_tests} ({100*total_pass/total_tests:.1%}) meets or exceeds 90% target"
            )
        else:
            report.append("⚠️ **REVIEW REQUIRED** - Some tests did not meet expectations")
            report.append(
                f"Pass rate {total_pass}/{total_tests} ({100*total_pass/total_tests:.1%}) is below 90% target"
            )
