...
class LRUTTLCache:
    ...
    def clear(self) -> int:
        """Evict all entries.  Returns number of entries cleared."""
        with self._lock:
            n = len(self._store)
            self._store.clear()
            return n
    ...
    def purge_expired(self) -> int:
        """Sweep and remove all expired entries.  Returns count removed."""
...
class CacheLayer:
    ...
    def __init__(
        ...
        self._caches: Dict[str, LRUTTLCache] = {
            ...
            "aggregate": LRUTTLCache(
                aggregate_max_size, aggregate_ttl, "aggregate"
            ),
            "attest": LRUTTLCache(
                attest_max_size, attest_ttl, "attest"
            ),
            "load_wasm": LRUTTLCache(
                wasm_max_size, wasm_ttl, "load_wasm"
            ),
        }

    # ── Operation-specific cache helpers ─────────────────────────────────────
    ...
    def verify_proof_batch(
        ...
        **fallback_kwargs,
        ...
        """
        Cache wrapper for verify_proof_batch.

        Key strategy: SHA-256(sorted proof bytes + public_inputs dict)
        Security: negative results (False / invalid) are NOT cached long-term
                  — they use the same TTL so stale negatives auto-expire.
        """
    ...
    def aggregate(
        ...
        **fallback_kwargs,
        ...
        """
        Cache wrapper for aggregate.

        Key strategy: SHA-256(frozenset of node_id + hash(weights) tuples)
        """
    ...
    def attest(
        ...
        **fallback_kwargs,
        ...
        """
        Cache wrapper for attest (de-dup within TTL=1s window).

        Key strategy: (node_id, floor(time_ns / 1e9)) — same-second de-dup.
        ⚠️  Security-sensitive — keep TTL <= 1s, same-process only.
        """
    ...
    def load_wasm(
        ...
        **fallback_kwargs,
        ...
        """
        Cache wrapper for load_wasm.

        Key strategy: SHA-256(file_path + checksum/mtime) — invalidates on change.
        """
