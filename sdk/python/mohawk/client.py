...
class MohawkNode:
    ...
    def verify_hybrid(
        ...
        **overrides: Any,
        ...
        request = (
            check
            if isinstance(check, HybridProofCheck)
            else HybridProofCheck.from_mapping(check)
        )
        ...
        *,
        ...
        if not result.get("success", False):
            raise AggregationError(
                result.get("message", "zero-copy compression failed")
            )
        return result
    ...
    def _router_request(
        ...
        *,
        ...
        try:
            ...
            raise AggregationError(
                f"router {method} {path} failed: {exc.code} {detail}"
            ) from exc
        except urllib.error.URLError as exc:
            raise AggregationError(
                f"router {method} {path} unreachable: {exc.reason}"
            ) from exc
