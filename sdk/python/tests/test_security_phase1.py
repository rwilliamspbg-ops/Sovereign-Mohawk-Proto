...
class TestEnvironmentProvider:
    ...
    async def test_get_credential_success(self):
        ...
        value = await provider.get_credential("TEST_CRED")
    ...
    async def test_get_credential_not_found(self):
        ...
        with pytest.raises(CredentialNotFoundError):
            await provider.get_credential("NONEXISTENT_CREDENTIAL")
    ...
    async def test_set_credential(self):
        ...
        await provider.set_credential("NEW_CRED", "new-value")
    ...
    async def test_delete_credential(self):
        ...
        await provider.delete_credential("DELETE_ME")
        assert "DELETE_ME" not in os.environ
    
    @pytest.mark.asyncio
    async def test_rotate_not_supported(self):
        """Test rotation not supported."""
        provider = EnvironmentProvider()
        
        with pytest.raises(CredentialRotationError):
            await provider.rotate_credential("ANY_KEY")
...
class TestCredentialManager:
    ...
    async def test_get_with_cache(self):
        ...
        value1 = await manager.get("CACHED_CRED")
        assert value1 == "value-123"
        
        # Second call uses cache
        value2 = await manager.get("CACHED_CRED")
    ...
    async def test_cache_bypass(self):
        ...
        value1 = await manager.get("BYPASS_TEST")
        ...
        value2 = await manager.get("BYPASS_TEST", use_cache=True)
        assert value2 == "original"
        
        # Without cache, should see updated
        value3 = await manager.get("BYPASS_TEST", use_cache=False)
    ...
    async def test_credential_not_found(self):
        ...
        with pytest.raises(CredentialNotFoundError):
            await manager.get("MISSING_CREDENTIAL")
...
class TestSecurityIntegration:
    ...
    @pytest.mark.asyncio
    async def test_secure_credential_workflow(self):
        ...
        api_token = await manager.get("API_TOKEN")
        assert api_token == "secret-token-12345"
        
        # Verify not in logs (would need to check log output)
        assert "secret-token-12345" not in str(manager._cache.get("API_TOKEN", ("", ""))[0])
...
class TestPerformance:
    ...
    @pytest.mark.asyncio
    async def test_credential_cache_performance(self):
        ...
        import time
        start = time.time()
        await manager.get("PERF_TEST", use_cache=True)
    ...
    def test_ssl_context_creation_performance(self):
        ...
        # Should be fast (< 100ms for 10 creations)
        assert duration < 0.1
...
class TestErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_credential_error_messages(self):
        """Test credential errors have clear messages."""
        provider = EnvironmentProvider()
        
        try:
            await provider.get_credential("MISSING")
