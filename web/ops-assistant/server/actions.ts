...
// Validation schemas
const validatePromQL = (query: string): boolean => {
...
export const queryMetricAction = {
  name: 'queryMetric',
  description:
    'Query Prometheus metrics with custom PromQL. Advanced metric queries with aggregation, filters, and time series analysis.',
  parameters: {
    type: 'object',
    properties: {
      query: {
        type: 'string',
        description:
          'PromQL query (e.g., "rate(http_requests_total[5m])" or "histogram_quantile(0.95, rate(http_request_duration_seconds_bucket[5m]))")',
      },
      timeRange: {
        type: 'string',
...
export const explainDashboardAction = {
  ...
  description:
    'Analyze and explain a Grafana dashboard, including panels, metrics, and data sources used.',
  parameters: {
    type: 'object',
    properties: {
      dashboardUid: {
  ...
  handler: async (params: any) => {
    try {
      ...
      const filteredAnnotations = annotations.filter((a: any) =>
        a.text.toLowerCase().includes(params.query.toLowerCase())
      );

      const filteredAlerts = alerts.filter(
        (a: any) =>
          a.name.toLowerCase().includes(params.query.toLowerCase()) ||
...
export const getNetworkTopologyAction = {
  ...
  handler: async (params: any) => {
    try {
      ...
      const nodes = response.data.data.result.map((r: any) => ({
        id: r.metric.job,
        label: r.metric.job,
      ...
      return {
        ...
        edges:
          nodes.length > 1
            ? nodes.slice(0, -1).map((n: { id: string }, i: number) => ({
                from: n.id,
                to: nodes[i + 1].id,
              }))
            : [],
...
export const analyzePerformanceAction = {
  ...
  description:
    'Comprehensive performance analysis of system components including latency, throughput, and resource usage.',
  parameters: {
    type: 'object',
    properties: {
      component: {
...
export const getRoundStatusAction = {
  ...
  description:
    'Get the current federated learning round status, including phase, progress, confidence, drift, and reasoning.',
  parameters: {
    type: 'object',
    properties: {
      includeEvidence: {
...
export const explainModelDriftAction = {
  ...
  description:
    'Explain why model drift is changing, which nodes are affected, and what operator action is recommended.',
  parameters: {
    type: 'object',
    properties: {
      objective: {
...
export const listContributingNodesAction = {
  ...
  description:
    'Rank contributing federated nodes by their value to the current objective and return reasoning for each.',
  parameters: {
    type: 'object',
    properties: {
      objective: {
...
export const detectAnomaliesAction = {
  ...
  description:
    'Detect drift, poisoning, Byzantine, attestation, or latency anomalies in the current federated round.',
  parameters: {
    type: 'object',
    properties: {
      severityThreshold: {
...
export const getIntelligenceScoreboardAction = {
  ...
  description:
    'Return the current federated intelligence scoreboard with confidence, drift, contributors, and anomaly summary.',
  parameters: {
    type: 'object',
    properties: {},
  },
...
export const getNetworkStatsAction = {
  ...
  description:
    'Get comprehensive network statistics including uptime, error rates, and health indicators.',
  parameters: {
    type: 'object',
    properties: {
      timeRange: {
