...
let _tanstack_pacer = require("@tanstack/pacer");
...
var IntelligenceAgent = class IntelligenceAgent extends _ag_ui_client.AbstractAgent {
	...
	*/
	run(input) {
		...
		return (0, rxjs.defer)(() => this.requestJoinCredentials$("run", input)).pipe((0, rxjs.switchMap)((credentials) => {
			if (credentials === null) return (0, rxjs.throwError)(() => /* @__PURE__ */ new Error("REST run request returned no credentials"));
	...
	*/
	connect(input) {
		...
		return (0, rxjs.defer)(() => this.requestJoinCredentials$("connect", input)).pipe((0, rxjs.switchMap)((credentials) => {
			if (credentials === null) return rxjs.EMPTY;
	...
	*/
			...
			try {
				const response = await fetch(this.buildRuntimeUrl(mode), {
					...
					body: JSON.stringify({
						...
						...mode === "connect" ? { lastSeenEventId: this.getReconnectCursor(input) } : {}
					}),
	...
	observeThread$(input, credentials, options) {
		return this.observeThreadSession$(input, credentials, options).pipe((0, rxjs_operators.catchError)((error) => {
			if (!this.isSocketReconnectExhaustedError(error)) return (0, rxjs.throwError)(() => error);
			return this.requestJoinCredentials$("connect", input).pipe((0, rxjs.switchMap)((refreshedCredentials) => refreshedCredentials === null ? rxjs.EMPTY : this.observeThread$(this.applyCanonicalRunIdentity(input, refreshedCredentials, { fallbackToInputRunId: options.streamMode === "run" }), refreshedCredentials, {
				...options,
				channelMode: "connect",
	...
	observeThreadSession$(input, credentials, options) {
		return (0, rxjs.defer)(() => {
			...
			const replayComplete$ = this.observeControlEvent$(input.threadId, channel$, REPLAY_COMPLETE_EVENT).pipe((0, rxjs_operators.ignoreElements)(), (0, rxjs_operators.share)());
			const streamIdle$ = this.observeControlEvent$(input.threadId, channel$, STREAM_IDLE_EVENT).pipe((0, rxjs_operators.shareReplay)({
				bufferSize: 1,
				refCount: true
			}));
			const streamIdleCompletion$ = options.streamMode === "connect" ? streamIdle$.pipe((0, rxjs_operators.take)(1)) : rxjs.EMPTY;
			const terminal$ = (0, rxjs.merge)(threadEvents$.pipe((0, rxjs_operators.ignoreElements)(), (0, rxjs_operators.endWith)(null), (0, rxjs_operators.take)(1)), streamIdleCompletion$);
			return (0, rxjs.merge)(this.joinThreadChannel$(channel$), this.observeSocketHealth$(socket$).pipe((0, rxjs_operators.takeUntil)(terminal$)), threadEvents$.pipe((0, rxjs_operators.takeUntil)(streamIdleCompletion$)), replayComplete$.pipe((0, rxjs_operators.takeUntil)(terminal$)), streamIdleCompletion$.pipe((0, rxjs_operators.ignoreElements)())).pipe((0, rxjs_operators.finalize)(() => this.cleanupOwned(ownChannel, ownSocket)));
		});
	}
	...
	observeThreadEvents$(threadId, channel$, options) {
		...
		}), (0, rxjs_operators.mergeMap)((payload) => this.createThreadNotifications(payload, {
		...
		})), (0, rxjs.dematerialize)());
	}
	observeControlEvent$(threadId, channel$, eventName) {
		return channel$.pipe((0, rxjs_operators.switchMap)(({ channel }) => this.observeChannelEvent$(channel, eventName)), (0, rxjs_operators.tap)((payload) => this.updateLastSeenEventIdFromControl(threadId, payload)));
	}
	...
	createThreadNotifications(payload, options) {
		if (payload.type === _ag_ui_client.EventType.RUN_FINISHED) return options.completeOnRunFinished ? [rxjs.Notification.createNext(payload), rxjs.Notification.createComplete()] : [rxjs.Notification.createNext(payload)];
		if (payload.type === _ag_ui_client.EventType.RUN_ERROR) {
			const errorMessage = payload.message ?? "Run error";
			return options.completeOnRunError ? [rxjs.Notification.createNext(payload), rxjs.Notification.createComplete()] : options.errorOnRunError ? [rxjs.Notification.createNext(payload), rxjs.Notification.createError(new Error(errorMessage))] : [rxjs.Notification.createNext(payload)];
		}
	...
	readControlEventId(payload) {
		...
		const latestEventId = payload.latestEventId;
...
var ProxiedCopilotRuntimeAgent = class ProxiedCopilotRuntimeAgent extends _ag_ui_client.HttpAgent {
	...
	constructor(config) {
		...
		this.runtimeMode = config.runtimeMode ?? _copilotkit_shared.RUNTIME_MODE_SSE;
	...
	*/
	...
	async connectAgent(parameters, subscriber) {
		if (this.runtimeMode !== _copilotkit_shared.RUNTIME_MODE_INTELLIGENCE) return super.connectAgent(parameters, subscriber);
		if (this.delegate) await this.delegate.detachActiveRun();
	...
	connect(input) {
		if (this.runtimeMode === _copilotkit_shared.RUNTIME_MODE_INTELLIGENCE) return this.#connectViaDelegate(input);
		return this.#connectViaHttp(input);
	}
	run(input) {
		if (this.runtimeMode === _copilotkit_shared.RUNTIME_MODE_INTELLIGENCE) return this.#runViaDelegate(input);
	...
	#connectViaHttp(input) {
		...
		return withAbortErrorHandling((0, _ag_ui_client.transformHttpEventStream)((0, _ag_ui_client.runHttpRequest)(`${this.runtimeUrl}/agent/${routedId}/connect`, this.requestInit(input))));
	}
	...
	async resolveDelegate() {
		...
		if (!this.delegate) {
			if (this.runtimeMode !== _copilotkit_shared.RUNTIME_MODE_INTELLIGENCE) throw new Error("A delegate is only created for Intelligence mode");
	...
	async ensureRuntimeMode() {
		...
		this.runtimeInfoPromise ??= this.fetchRuntimeInfo().then((runtimeInfo) => {
			this.runtimeMode = runtimeInfo.mode ?? _copilotkit_shared.RUNTIME_MODE_SSE;
...
var AgentRegistry = class {
	...
	_runtimeMode = _copilotkit_shared.RUNTIME_MODE_SSE;
	_intelligence;
	_a2uiEnabled = false;
	...
	*/
	registerProxiedAgent({ agentId, runtimeAgentId }) {
		...
		const agent = new ProxiedCopilotRuntimeAgent({
			...
			runtimeMode: this._runtimeUrl ? this._runtimeConnectionStatus === CopilotKitCoreRuntimeConnectionStatus.Connected ? this._runtimeMode : "pending" : _copilotkit_shared.RUNTIME_MODE_SSE,
			intelligence: this._intelligence,
			debug: debug ? (0, _copilotkit_shared.resolveDebugConfig)(debug) : void 0
		});
		this.applyHeadersToAgent(agent);
	...
	*/
	async updateRuntimeConnection() {
		...
		if (!this.runtimeUrl) {
			...
			this._runtimeMode = _copilotkit_shared.RUNTIME_MODE_SSE;
		...
		try {
			...
			this.remoteAgents = Object.fromEntries(Object.entries(runtimeInfo.agents).map(([id, { description, capabilities }]) => {
				const agent = new ProxiedCopilotRuntimeAgent({
					...
					transport: this._runtimeTransport,
					credentials,
					runtimeMode: runtimeInfoResponse.mode ?? _copilotkit_shared.RUNTIME_MODE_SSE,
					intelligence: runtimeInfoResponse.intelligence,
					capabilities,
					debug: rawDebug ? (0, _copilotkit_shared.resolveDebugConfig)(rawDebug) : void 0
				});
				this.applyHeadersToAgent(agent);
				return [id, agent];
			}));
			...
			this._runtimeMode = runtimeInfoResponse.mode ?? _copilotkit_shared.RUNTIME_MODE_SSE;
			...
			_copilotkit_shared.logger.warn(`Failed to load runtime info (${this.runtimeUrl}/info): ${message}`);
	...
	*/
	async fetchRuntimeInfoAutoDetect(headers, credentials) {
		...
		const result = await this.fetchRuntimeInfoSingle({ ...headers }, credentials);
...
var ContextStore = class {
	...
	*/
	addContext({ description, value }) {
		const id = (0, _copilotkit_shared.randomUUID)();
		this._context[id] = {
			description,
			value
		};
...
var SuggestionEngine = class {
	...
	*/
	initialize(suggestionsConfig) {
		for (const config of suggestionsConfig) this._suggestionsConfig[(0, _copilotkit_shared.randomUUID)()] = config;
	}
	/**
	...
	*/
	reloadSuggestions(agentId) {
		...
		for (const config of Object.values(this._suggestionsConfig)) {
			...
			const suggestionId = (0, _copilotkit_shared.randomUUID)();
	...
	extractSuggestions(messages, suggestionId, consumerAgentId, isRunning) {
		...
		for (const message of newMessages) if (message.role === "assistant" && message.toolCalls) {
			for (const toolCall of message.toolCalls) if (toolCall.function.name === "copilotkitSuggest") {
				const parsed = (0, _copilotkit_shared.partialJSONParse)(Array.isArray(toolCall.function.arguments) ? toolCall.function.arguments.join("") : toolCall.function.arguments);
...
var RunHandler = class {
	...
	*/
	addTool(tool) {
		if (this._tools.findIndex((t) => t.name === tool.name && t.agentId === tool.agentId) !== -1) {
			_copilotkit_shared.logger.warn(`Tool already exists: '${tool.name}' for agent '${tool.agentId || "global"}', skipping.`);
			return;
		}
	...
	*/
	async runTool(params) {
		...
		const toolCallId = (0, _copilotkit_shared.randomUUID)();
...
function createToolSchema(tool) {
	...
	const rawSchema = (0, _copilotkit_shared.schemaToJsonSchema)(tool.parameters, { zodToJsonSchema: zod_to_json_schema.zodToJsonSchema });
...
function parseToolArguments(rawArgs, toolName) {
	if (rawArgs === "" || rawArgs === null || rawArgs === void 0) {
		_copilotkit_shared.logger.debug(`[parseToolArguments] Tool "${toolName}" received empty/null/undefined arguments — defaulting to {}`);
		return {};
	}
	...
	subscribeToAgent(agent) {
		...
		const { unsubscribe } = agent.subscribe({
			onRunStartedEvent: ({ input, state }) => {
				if (revoked) return;
				if (runFinished && input.runId === subRunId) subRunId = (0, _ag_ui_client.randomUUID)();
...
var CopilotKitCore = class {
	...
	*/
	subscribeToAgentWithOptions(agent, subscriber, options) {
		...
		const throttler = new _tanstack_pacer.Throttler(flushPending, {
			wait: effectiveMs,
			leading: true,
			trailing: true
		});
...
exports.AgentThreadLockedError = AgentThreadLockedError;
...
exports.ɵMAX_SOCKET_RETRIES = ɵMAX_SOCKET_RETRIES;
