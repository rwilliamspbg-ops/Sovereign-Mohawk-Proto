...
async function fetchFederationSnapshot() {
    try {
        ...
        return {
            status: {
                ...
                phase: payload.phase || DEFAULT_ROUND.status.phase,
                ...
                convergenceTrend: payload.convergenceTrend || DEFAULT_ROUND.status.convergenceTrend,
            ...
            contributors: Array.isArray(payload.contributors)
                ? payload.contributors.map((contributor) => ({
                    ...
                    attestationStatus: contributor.attestationStatus || 'unverified',
            ...
            anomalies: Array.isArray(payload.anomalies)
                ? payload.anomalies.map((anomaly) => ({
                    ...
                    category: anomaly.category || 'drift',
...
function buildReasoningTrail(status, contributors, anomalies) {
...
export async function listContributingNodes() {
    const snapshot = await federatedLearningStateMachine.refresh();
 ...
 */
export function computeDriftScoreFromContributors(contributors) {
 ...
 */
export function detectAnomalyHeuristics(snapshot) {
    const anomalies = [];
    if (!snapshot)
        return anomalies;
...
export async function explainModelDrift() {
    const snapshot = await federatedLearningStateMachine.refresh();
...
export async function getIntelligenceScoreboard() {
    const snapshot = await federatedLearningStateMachine.refresh();
    const topContributors = [...snapshot.contributors].sort((left, right) => right.contributionScore - left.contributionScore);
...
//# sourceMappingURL=federated-intelligence.js.map